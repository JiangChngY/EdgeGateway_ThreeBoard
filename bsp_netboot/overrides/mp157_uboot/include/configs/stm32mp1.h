...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"edge_serverip=192.168.88.131\0" \
	"edge_ip_method=dhcp\0" \
	"edge_gatewayip=192.168.88.2\0" \
	"edge_netmask=255.255.255.0\0" \
	"edge_hostname=mp157-edge\0" \
	"edge_nfsroot=/srv/nfs/edgegateway/mp157\0" \
	"edge_kernel=mp157/uImage\0" \
	"edge_fdt=mp157/stm32mp157d-atk-edgegateway.dtb\0" \
	"edge_get_ip=" \
	...
	"edge_netargs=" \
		...
		"fi; " \
		"setenv bootargs console=ttySTM0,115200 root=/dev/nfs rw " \
		"nfsroot=${edge_serverip}:${edge_nfsroot},vers=3,tcp ${edge_ip_arg} rootwait\0" \
	"edge_netboot=" \
		"echo Booting EdgeGateway MP157 from TFTP/NFS...; " \
		"run edge_get_ip; " \
		"if tftpboot ${kernel_addr_r} ${edge_kernel}; then " \
			"if tftpboot ${fdt_addr_r} ${edge_fdt}; then " \
				"run edge_netargs; " \
				"bootm ${kernel_addr_r} - ${fdt_addr_r}; " \
			"else echo ERROR: MP157 device tree download failed; fi; " \
		"else echo ERROR: MP157 kernel download failed; fi\0" \
